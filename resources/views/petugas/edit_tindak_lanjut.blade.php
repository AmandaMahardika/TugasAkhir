...
<body class="relative bg-gradient-to-br from-sky-200 to-white min-h-screen py-12 px-4 overflow-hidden flex items-center justify-center">
    ...
    <div class="relative z-10 bg-white/90 backdrop-blur-md rounded-2xl shadow-xl w-full max-w-2xl mx-auto p-10 fade-up">
        ...
        <!-- Header dengan gambar di samping judul -->
        <div class="flex items-center justify-center mb-8 gap-8">
            <img src="{{ asset('images/undraw_text-field_17if.svg') }}" alt="Edit Icon" class="w-32 h-auto" />
            <h1 class="text-3xl font-semibold text-sky-700">Edit Tindak Lanjut</h1>
        </div>
        ...
        <form action="{{ route('tindakLanjut.update', $tindakLanjut->id) }}" method="POST" class="space-y-8">
            ...
            <div>
                <label for="penanganan" class="block text-sky-700 font-semibold mb-2">Penanganan:</label>
                <textarea id="penanganan" name="penanganan" rows="4" class="w-full rounded-lg border border-sky-300 px-4 py-3 text-gray-700 focus:outline-none focus:ring-2 focus:ring-sky-400 resize-y">{{ old('penanganan', $tindakLanjut->penanganan) }}</textarea>
                @error('penanganan')
            ...
            <div>
                <label for="status" class="block text-sky-700 font-semibold mb-2">Status:</label>
                <select id="status" name="status" class="w-full rounded-lg border border-sky-300 px-4 py-3 text-gray-700 focus:outline-none focus:ring-2 focus:ring-sky-400">
            ...
            <div class="flex justify-between gap-4 mt-6">
                <button type="submit" class="flex items-center justify-center gap-2 bg-sky-500 hover:bg-sky-700 text-white font-semibold py-3 px-8 rounded-lg transition focus:outline-none focus:ring-2 focus:ring-sky-400">
